...
### Power BI Project: Phase 1 (ETL), Phase 2 (Model), Phase 3 (DAX Measures)

---

> This document explains where each field in the Power BI report comes from, what it was renamed to, what new fields were created, what was removed, and why certain data formats were chosen. Power Query loaded columns are listed by table. Phase 3 adds calculated columns on `DimProduct` (documented below). Full measure DAX and build order: [**dax-measures.md**](./dax-measures.md).
...
**Format:** CSV - 53 columns, 180,519 rows  

**Original Language:** Spanish (translated to English - some column names are translation artifacts)  
...
| 6   | Delivery Status               | Text           | Shipping outcome label: Advance shipping, Late delivery, Shipping on time, Shipping canceled                                                                                                                                                                                                                  |
| 7   | Late_delivery_risk            | Whole Number   | Source Machine Learning training label replaced by **Late Delivery Risk** and **Is Late Delivery Risk** in `FactOrders`, both derived from `Lead Time Variance > 0` (actual vs. scheduled shipping days). See `[decision-log.md](./decision-log.md)` Entry #3. FactOrders column definitions below. |
...
## DimDate - 23 Columns, 2,191 Rows

**Source:** Generated from MasterSet-derived min/max boundaries (not a reference query)  

**Date Range:** Jan 1, 2014 → Dec 31, 2019  

**Padding:** 1 year before min transaction date, 1 year after max transaction date  

**Grain:** One row per calendar day  

**Primary Key (PK):** Date  

**Foreign Key (FK):** None (dimension table)  
...
## DimProduct - 118 Rows (11 Power Query columns + Phase 3 calculated columns)

**Source:** Reference query from MasterSet. Calculated columns added in **Phase 3** (Measures) at refresh  

**Grain:** One row per unique product  

**Primary Key (PK):** Product Card Id  

**Foreign Key (FK):** None (dimension table)  

**Duplicate Check Field:** Product Card Id (removed duplicate rows, kept one row per Product Card Id)

### Loaded columns (Power Query)
...
Evaluated at **model refresh**. Folder in model: **Inventory Segmentation**. Full formulas and measure equivalents: `[dax-measures.md](./dax-measures.md)` - *DimProduct Calculated Columns*. See architectural decision entries: `[decision-log.md](./decision-log.md)`.
...
| `Simulated Inventory Level`                              | Whole Number | **Test column only**. Simulated on-hand value for reorder logic testing. Replace with live WMS/ERP snapshot in production. See Entry #21.                                                                                                |
| `Demand Velocity Band (DEPRECATED - XYZ Classification)` | Text         | **Deprecated**. Superseded by `XYZ Classification (DimColumn)`. Retained for audit trail. See Entry #17.                                                                                                                                                                                   |
...
**Source:** Reference query from MasterSet  

**Grain:** One row per unique customer  

**Primary Key (PK):** Customer Id  
...
| Delivery Status | Text           | Delivery Status | No       | **Late delivery**, **Advance shipping**, **Shipping on time**, **Shipping canceled** (strings used in DAX filters; supply metrics exclude **Shipping canceled**). Entry #19. |
...
*Document Version: 1.3 — Phases 1–3: ETL + model layer + DimProduct calculated columns*